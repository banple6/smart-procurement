...
def delivery_sheets_workbook(units: list[dict]) -> bytes:
    ...
    product_totals: dict[tuple[str, str, str, str], dict[str, float | int]] = defaultdict(
    ...
    for unit in units:
        for order in unit["orders"]:
            for item in order["items"]:
                ...
                product_totals[key]["requested_quantity"] = float(product_totals[key]["requested_quantity"]) + float(item["requested_quantity"])
                product_totals[key]["actual_quantity"] = float(product_totals[key]["actual_quantity"]) + float(item["actual_quantity"])
