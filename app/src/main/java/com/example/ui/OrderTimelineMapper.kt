...
fun orderTimelineNodes(order: OrderEntity): List<StatusStep> {
    ...
    if (order.status == "已作废") {
        return listOf(
            StatusStep("订单已提交", StepState.COMPLETED, completedTime(created)),
            StatusStep("订单已作废", StepState.CANCELLED, "作废后库存已释放")
        )
    }
    ...
    return when (order.status) {
        "已完成" -> listOf(
            ...
            StatusStep("订单备货", StepState.COMPLETED, completedTime(order.preparingAt)),
        ...
        "备货中" -> listOf(
            StatusStep("订单已提交", StepState.COMPLETED, completedTime(created)),
            StatusStep("管理员已接单", StepState.COMPLETED, completedTime(order.acceptedAt)),
            StatusStep("订单备货", StepState.CURRENT, order.preparingAt.ifBlank { null }),
        ...
        "已接单" -> listOf(
            StatusStep("订单已提交", StepState.COMPLETED, completedTime(created)),
            StatusStep("管理员已接单", StepState.COMPLETED, completedTime(order.acceptedAt)),
            StatusStep("正在同步备货状态", StepState.CURRENT),
            StatusStep("已发货", StepState.UPCOMING),
            StatusStep("已完成", StepState.UPCOMING)
        )
        else -> listOf(
            ...
            StatusStep("订单备货", StepState.UPCOMING),
            StatusStep("已发货", StepState.UPCOMING),
            StatusStep("已完成", StepState.UPCOMING)
        )
    }
